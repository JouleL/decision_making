...
def condorcet_method(data, benefits):
    votes = {}
    ...
    for result in votes:
        ...
        largest_candidate = candidates if votes[result] > votes[candidates[1] + '>' + candidates[0]] else [
            candidates[1], candidates[0]]
        key = largest_candidate[0] + '>' + largest_candidate[1]
        final_eloctorates[key] = votes[key]
    print("Найбільші серед вибірки:", final_eloctorates)

    for e in final_eloctorates.keys():
        candidates = re.split('>', e)
        if candidates[0] not in benefits_first_place:
            continue
